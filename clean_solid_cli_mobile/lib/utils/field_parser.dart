...
class FieldParser {
  static List<Field> parse(String input) {
    ...
    for (var part in parts) {
      ...
      if (refMatch != null) {
        ...
        fields.add(Field(
          name: name, type: 'String', isReference: true,
      ...
      if (enumMatch != null) {
        ...
        final className = name.split('_').map((w) {
          if (w.isEmpty) return '';
          return '${w[0].toUpperCase()}${w.substring(1)}';
        }).join();
        fields.add(Field(name: name, type: className, isEnum: true, enumValues: values));
      } else {
  ...
  static String _mapDartType(String input) {
    switch (input.toLowerCase()) {
      ...
      case 'num': return 'num';
      default:
        print('  ⚠  Type inconnu "$input" → mappé à dynamic.');
        print('     Types supportés : string, int, double, decimal, bool, boolean, datetime, date, num');
        return 'dynamic';
    }
  }
}
